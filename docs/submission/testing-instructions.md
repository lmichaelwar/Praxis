...
The browser may prompt for the demo credential once. The Studio, API, and media are protected by the same demo-only access boundary. No separate account creation or paid API key is required.

## Clean test

1. Open the live URL in a clean browser profile or incognito window.
2. Enter the demo credential when prompted.
3. Wait for the header to show a project revision, **Synced to control plane**, and **WebMCP · 6 bounded tools**.
